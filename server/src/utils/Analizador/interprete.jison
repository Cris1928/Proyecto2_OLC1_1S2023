...
/* Ejemplo para la gramatica del interprete err semantico*/

/* Definicion lexica String*/
%lex
...
"println"                 { console.log("Reconocio : " + yytext);  return 'PRINT' }
"print"                 { console.log("Reconocio : " + yytext);  return 'PRINTLN' }

"if"               { console.log("Reconocio : "+ yytext); return 'IF'}
"else if"            { console.log("Reconocio : "+ yytext); return 'ELSEIF'}
"while"            { console.log("Reconocio : "+ yytext); return 'WHILE'}
"do"            { console.log("Reconocio : "+ yytext); return 'DO'}
"until"            { console.log("Reconocio : "+ yytext); return 'UNTIL'}
...
%left 'OR'
%left 'AND'
%right 'NOT' 'CASTEODOUBLE' 'CASTEOINT' 'CASTEOSTRING' 'CASTEOCHAR' 'CASTEOTIPO' 'CASTEOTOLOWER' 'CASTEOTOUPPER' 'LENGTH' 'ROUND' 'TRUNCATE'
...
declaracion : tipo lista_ids IGUAL e PYC  { $$ = new declaracion.default($1, $2, $4,@1.first_line, @1.last_column);}  
            ...
            | tipo CORA CORC lista_ids IGUAL NEW  tipo CORA e CORC  PYC         { $$ = new declaracion.default($1, $4, null,  @1.first_line, @1.last_column,$7,$9);}
            | tipo CORA CORC lista_ids IGUAL LLAVA listasimple  LLAVC  PYC         { $$ = new declaracion.default($1, $4, null,  @1.first_line, @1.last_column,$1,$4);}
            | tipo CORA CORC  lista_ids IGUAL e PYC { $$ = new declaracion.default($1, $4, $6,  @1.first_line, @1.last_column);}
            | LIST MENORQUE tipo MAYORQUE lista_ids IGUAL NEW LIST MENORQUE tipo MAYORQUE PYC        { $$ = new declaracion.default($3, $5, null,  @1.first_line, @1.last_column,$3,null);}
            ;
...
sent_if : IF PARA e PARC LLAVA instrucciones LLAVC{ $$ = new Ifs.default($3, $6, [], @1.first_line, @1.last_column); }
        ...
        | ELSEIF PARA e PARC LLAVA instrucciones LLAVC { $$ = new Ifs.default($3, $6, [], @1.first_line, @1.last_column); }
        | ELSEIF PARA e PARC LLAVA instrucciones LLAVC ELSE LLAVA instrucciones LLAVC { $$ = new Ifs.default($3, $6, $10, @1.first_line, @1.last_column); }
        ;
...
sent_Dowhile : DO  LLAVA instrucciones LLAVC WHILE PARA e PARC PYC  { $$ = new DoWhile.default($7, $3, @1.first_line, @1.last_column);  }
        | DO  LLAVA instrucciones LLAVC UNTIL PARA e PARC PYC  { $$ = new DoWhile.default($7, $3, @1.first_line, @1.last_column);  }
            ;
...
funciones 
        : tipo ID PARA lista_params PARC LLAVA instrucciones LLAVC   { $$ = new funcion.default(2, $1, $2, $4, false, $7,  @1.first_line, @1.last_column); }
...
startwith :  MAIN  llamadaeee PYC    { $$ = new startwith.default($2,@1.first_line, @1.last_column );}
        ;
...
e : e MAS e         { $$ = new aritmetica.default($1, '+', $3, @1.first_line, @1.last_column,false); }
    ...
    | TRUNCATE PARA e PARC { $$ = new logica.default($3, '(truncate)', null, @1.first_line, @1.last_column,true); }
